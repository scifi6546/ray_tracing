...
pub trait PDF {
    ...
    fn generate(&self, incoming_ray: Ray, world: &World) -> (Vector3<f32>, f32);
}
pub struct CosinePdf {
    pub uvw: OrthoNormalBasis,
}
impl PDF for CosinePdf {
    fn generate(&self, _ray: Ray, _world: &World) -> (Vector3<f32>, f32) {
...
impl PDF for LightPdf {
    fn generate(&self, incoming_ray: Ray, world: &World) -> (Vector3<f32>, f32) {
        ...
        let ray = world.lights[idx].generate_ray_in_area(incoming_ray.origin, incoming_ray.time);
        let value = world.lights[idx].prob(ray);
        (ray.direction.normalize(), value)
    }
}
