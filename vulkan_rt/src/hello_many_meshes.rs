...
pub fn run(base: &Base) {
    ...
    let index_buffer_memory_req =
        unsafe { base.device.get_buffer_memory_requirements(index_buffer) };
    let index_buffer_memory = allocator
        .allocate(&AllocationCreateDesc {
            name: "index buffer memory",
            requirements: index_buffer_memory_req,
            location: MemoryLocation::CpuToGpu,
        ...
        .expect("failed to allocate");
    let index_ptr = index_buffer_memory.mapped_ptr().expect("failed to map ptr");
    ...
    unsafe {
        ...
        base.device
            .bind_buffer_memory(
                index_buffer,
                index_buffer_memory.memory(),
                index_buffer_memory.offset(),
            )
            .unwrap();
    }
    ...
    let vertex_buffer_allocation = allocator
        ...
        .expect("failed to allocate");
    let vert_ptr = vertex_buffer_allocation
        .mapped_ptr()
        .expect("failed to map vertex ptr");
    ...
    unsafe {
        ...
        allocator.free(index_buffer_memory).expect("failed to free");
