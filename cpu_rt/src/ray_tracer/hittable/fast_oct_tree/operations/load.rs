...
impl FastOctTree<Voxel> {
    pub fn load_map<P: AsRef<Path>>(load_path: P) -> Result<Self, IoError> {
        ...
        for x in 0..map.tiles.len() {
            for z in 0..map.tiles[x].len() {
                ...
                if let Some(tile) = tile.as_ref() {
                    let x_tile_pos = map.tiles.len() - x - 1;
    ...
    pub(crate) fn load_vox<P: AsRef<Path>>(load_path: P) -> Result<Self, IoError> {
