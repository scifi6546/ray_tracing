            ...
            if (positions.length >= 1) {
                ...
                while (n < 10) {
                    ...
                    try {
                        ...
                        console.error("invalid block x, block_y")
                        step_size = 2;
                    }
                    // step_size = this.getStepSize(block_x, block_y);


                    let t_x = (block_x + Math.max(step_size * Math.sign(direction.x), 0.) - position.x) / direction.x;
                    let t_y = (block_y + Math.max(step_size * Math.sign(direction.y), 0.) - position.y) / direction.y;
                    ...
                    if (t_x < 0 || t_y < 0) {
                        ...
                        throw "t_x or t_y negative"
                    }
                    if (t_x < t_y) {
                        min_t = t_x;
                        block_x += step_size * Math.sign(direction.x);
                    } else {
                        min_t = t_y;
                        block_y += step_size * Math.sign(direction.y);
                    }

                    position.x += direction.x * min_t;
                    position.y += direction.y * min_t;

                    if (block_x < 0 || block_x >= this.getSideLength() || block_y < 0 || block_y >= this.getSideLength()) {
                        n = 1000000000;
                        break

                    } else {
                        output.push(new Vec2(position.x, position.y))

                        //debug_text += `\nt_x:${t_x},t_y: ${t_y}\nn_block: <${next_block_x}, ${next_block_y}>\ndirection sign: <${Math.sign(direction.x)},${Math.sign(direction.y)}>`
                        n += 1;
                    }


                }
                return [[output_position].concat(output), { "debug_text": debug_text, "color": node.getColor() }]
            } else {
                return [[], { "debug_text": "" }]
            }
